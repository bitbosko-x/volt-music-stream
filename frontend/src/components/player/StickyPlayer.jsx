...
import {
    ...
    ListPlus, CheckCircle2, Disc, Flag
} from 'lucide-react';
...
export function StickyPlayer() {
    ...
    useEffect(() => {
        ...
        const handlePlayerUpdate = async (e) => {
            ...
            try {
                const { stream_url, source } = await getAudioStream(trackData.search_term, trackData.artist, trackData.saavn_id, trackData.album);
    ...
    // Restore playback position AND kick off play when a new stream URL arrives
    useEffect(() => {
        const audio = audioRef.current;
        if (!audio || !player?.stream_url) return;

        const handleLoadedMetadata = () => {
            // Restore seek position for page-reload continuity
            const savedTime = localStorage.getItem('currentTime');
            const wasPlaying = localStorage.getItem('wasPlaying') === 'true';
            if (savedTime) {
                const time = parseFloat(savedTime);
                // Only restore position if this is a page reload of the same track
                // (not a fresh play click — those always start from 0)
                if (time > 0 && wasPlaying) {
                    audio.currentTime = time;
                    setCurrentTime(time);
                }
            }
            // Always try to play when a new stream URL loads
            audio.play().catch(e => console.log('Play on loadedmetadata:', e));
        };

        audio.addEventListener('loadedmetadata', handleLoadedMetadata);
        return () => audio.removeEventListener('loadedmetadata', handleLoadedMetadata);
    }, [player?.stream_url]);  // Re-run whenever the actual URL changes, not just player object
    ...
    const playTrackAtIndex = useCallback(async (index) => {
        ...
        setStreamLoading(true);
        try {
            const { stream_url, source } = await getAudioStream(song.search_term, song.artist, song.saavn_id, song.album);
            const trackData = {
                title: song.title,
                artist: song.artist,
                ...
                queue: queue,
                currentIndex: index,
                playedIndex: index,  // keeps Home.jsx song-card highlight in sync during prev/next
            };
            setPlayer(trackData);
        ...
        <>
            <audio
                ...
                onPlay={() => {
                    ...
                    if (audioCtxRef.current && sourceRef.current && sourceRef.current.mediaElement !== audioRef.current) {
                        try {
                    ...
                    if (!audioCtxRef.current && audioRef.current) {
                        try {
                            ...
                            // ── Global unlock: resume context on ANY user interaction ──
                            const resumeCtx = () => {
                            ...
                            console.error("Audio Context Init Failed (possibly CORS or re-bind issue)", e);
                        }
                ...
                <div className="fixed inset-0 z-[120] bg-[#09090b] text-white flex flex-col font-sans animate-in fade-in duration-200 h-[100dvh] overflow-hidden">
                    ...
                    <footer className="bg-[#121212] border-t border-white/5 flex flex-col md:grid md:grid-cols-[1fr_2fr_1fr] items-center px-4 md:px-6 py-4 md:py-0 md:h-24 gap-4 md:gap-0 z-40 relative">
                        ...
                        <div className="flex items-center justify-end gap-3 min-w-0">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => setIsReportModalOpen(true)} className="rounded-full hover:bg-white/10 text-zinc-400 hover:text-white" title="Report Audio Mismatch">
                    ...
                    <div className="bg-gradient-to-r from-slate-900/98 via-purple-900/98 to-slate-900/98 backdrop-blur-xl border-t border-purple-500/20">
                        ...
                        <div className="max-w-screen-2xl mx-auto flex items-center gap-2 px-3 py-4 md:gap-4 md:px-5 md:py-4 cursor-pointer" onClick={(e) => {
                            ...
                            <div className="flex items-center justify-end shrink-0 gap-1 md:gap-3">
                                ...
                                <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); setIsReportModalOpen(true); }} className="h-8 w-8 md:h-9 md:w-9 text-purple-200 hover:bg-red-500/20 hover:text-red-400" title="Report Audio Mismatch">
